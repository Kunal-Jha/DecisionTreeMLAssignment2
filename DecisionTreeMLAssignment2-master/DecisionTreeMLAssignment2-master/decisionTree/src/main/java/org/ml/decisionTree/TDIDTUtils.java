...
//			Double val1 =  sortedAttributeColumnList.get(i).get(1); // picking class label of first element
			if (sortedAttributeColumnList.get(i).get(1).intValue() != (sortedAttributeColumnList
					.get(i + 1).get(1).intValue())) { // if flip in class value 0-1 or 1-0
				Double x = sortedAttributeColumnList.get(i).get(1);
				Double y = sortedAttributeColumnList.get(i+1).get(1);
				if(x==y)
				{
					x = -y;
				}
				setOne = sortedAttributeColumnList.subList(0, i+1);
				...
				double leftSide = getEntropy(setOnePositiveCount + setTwoPositiveCount, setOneNegativeCount + setTwoNegativeCount);
				//double RightSide = (getEntropy(setOnePositiveCount, setOneNegativeCount) * (i + 1) / totalSize) + (getEntropy(setTwoPositiveCount, setTwoNegativeCount)*(totalSize - i) / totalSize);
				double RightSide = ((double)(getEntropy(setOnePositiveCount, setOneNegativeCount) * (double)(setOnePositiveCount + setOneNegativeCount) / totalSize) + ((double)getEntropy(setTwoPositiveCount, setTwoNegativeCount)*(double)(setTwoPositiveCount + setTwoNegativeCount) / totalSize));
				
	//			double informationGain = getEntropy(setOnePositiveCount + setTwoPositiveCount, setOneNegativeCount + setTwoNegativeCount)
	//- ((getEntropy(setOnePositiveCount, setOneNegativeCount) * (i + 1) / totalSize) + (getEntropy(setTwoPositiveCount, setTwoNegativeCount)*(totalSize - i) / totalSize));
				double informationGain = leftSide - RightSide;
	...
	public Double getEntropy(int positiveCount, int negativeCount) {
		...
		if (totalCount != 0 && positiveCount != 0 && negativeCount != 0) {
			/*double l2 = (double) (log2(positiveCount / totalCount));
			double l = ((double)positiveCount / totalCount * (double) log2((double) positiveCount / totalCount));
			double r = ((negativeCount / totalCount) * log2(negativeCount / totalCount));
			*/
			return -((((double)positiveCount / totalCount)* log2((double)positiveCount / totalCount)) + (((double)negativeCount / totalCount) * log2((double)negativeCount / totalCount)));
			//return (((double)positiveCount / totalCount)* log2((double)totalCount / positiveCount) + ((double)negativeCount / totalCount) * log2((double)totalCount / negativeCount));
		} else {
			return 0.0;
		}
	}
