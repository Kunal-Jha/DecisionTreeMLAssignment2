...
public class tdidt {
	...
	public static void main(String[] args){
		tdidt test = new tdidt();
	}
	
	public tdidt() {
		// TODO Auto-generated constructor stub
		trainInput trainingInput = new trainInput("./assets/gene_expression_training.csv");
		trainingInput.readData();
		ArrayList<ArrayList<Double>> examplesList  = trainingInput.getInput();
		ArrayList<String> attributesList = trainingInput.getAttributes();
		Node node = graph.addNode(String.valueOf(nodeIndex));
		nodeIndex++;
		
		Graph T = tdidt_recursive(examplesList, attributesList, graph, node);
		 T.display();
	}
	
	public ArrayList<ArrayList<Double>> getSortedExamples(ArrayList<ArrayList<Double>> examplesList, final int columnIndex){
	...
	public Graph tdidt_recursive(ArrayList<ArrayList<Double>> examplesList, ArrayList<String> attributesList, Graph T, Node node){
		
		// if the examples are perfectly classified	
		
		double c = examplesList.get(0).get(examplesList.get(0).size()-1);
		if(isPerfectlyClassified(examplesList)){
		...
		for (int i = 0; i < attributesList.size()-1; i++) {	
			Tuple minInformationGainInfo = getSplitPointAtMinimumInformationGainForAttribute(getAttributePlusClassifierColumnList(examplesList, i));
			
			Double currentInformationGain = minInformationGainInfo.getInformationGain();
		...
		node.setAttribute("attribute", attributesList.get(selectedAttributeIndex));
		
		// 7 in algo. Two possibilities for tree to grow. We know the split point and the attribute being analysed
		...
		for (int i = 0; i < examplesList.size(); i++) {
			if(examplesList.get(i).get(selectedAttributeIndex) < splitPointValue){
				examplesList.get(i).remove(selectedAttributeIndex) ;//removing the already evaluated attribute.
				lessThanBranchExampleList.add(examplesList.get(i));
			} else {
				examplesList.get(i).remove(selectedAttributeIndex);
				greaterThanBranchExampleList.add(examplesList.get(i));
			}
		}
		...
		tdidt_recursive(lessThanBranchExampleList, attributesList, T, lessThanNode);
		tdidt_recursive(greaterThanBranchExampleList, attributesList, T, greaterThanNode);
	...
	/**
	 * 
	 * @param attributeColumnList examples s.t. each row has a single attribute and a classifier value
	 * @return the split point that gives minimum information gain
	 */
	private Tuple getSplitPointAtMinimumInformationGainForAttribute(ArrayList<ArrayList<Double>> attributeColumnList) {
		
		ArrayList<ArrayList<Double>> sortedAttributeColumnList = getSortedExamples(attributeColumnList, 0);
		...
		for (int i = 0; i < totalSize-1; i++) {
			...
			if(sortedAttributeColumnList.get(i).get(1) != sortedAttributeColumnList.get(i+1).get(1)){ // if flip in class value 0-1 or 1-0
				setOne =  sortedAttributeColumnList.subList(0, i); 
				setTwo =  sortedAttributeColumnList.subList(i+1, totalSize-1);
				...
				double informationGain = getEntropy(setOnePositiveCount+setTwoPositiveCount, setOneNegativeCount+setTwoNegativeCount) - 
						((getEntropy(setOnePositiveCount, setOneNegativeCount) * (i+1) / totalSize) + 
								(getEntropy(setTwoPositiveCount, setTwoNegativeCount) * (totalSize - i) / totalSize)) ;
				
				double splitPoint = (sortedAttributeColumnList.get(i).get(0) + sortedAttributeColumnList.get(i+1).get(0))/2.0;
	...
	private Double getEntropy(int positiveCount, int negativeCount){
		...
		System.out.println(totalCount + " " + positiveCount + " " + negativeCount );
		if(totalCount!=0 && positiveCount!=0 && negativeCount!=0 ){
			return ( (positiveCount/totalCount)*log2(totalCount/positiveCount) +  (negativeCount/totalCount)*log2(totalCount/negativeCount));
		} else {
			return 0.0;
		}
	}
	...
	private ArrayList<ArrayList<Double>> getAttributePlusClassifierColumnList(ArrayList<ArrayList<Double>> examplesList, int columnIndex){
	...
	private ArrayList<Double> getAttributeColumnList(ArrayList<ArrayList<Double>> examplesList, int columnIndex){
	...
	private boolean isPerfectlyClassified(ArrayList<ArrayList<Double>> examplesList) {
		double c = examplesList.get(0).get(examplesList.get(0).size()-1);
